...
.PHONY: test
test:
	poetry run pytest --cov-report=xml --cov-report term-missing --cov=rhubarb -c pyproject.toml tests 

.PHONY: check-codestyle
check-codestyle:
	poetry run isort --diff --check-only --settings-path pyproject.toml ./
	poetry run black --diff --check --config pyproject.toml ./
	poetry run darglint --verbosity 2 rhubarb tests
...
check-safety:
	...
	poetry run bandit -ll --recursive rhubarb tests
